...
vec3 La = vec3 (0.0, 0.2, 0.2); // grey ambient colour
...
void main()
{
	...
	vec3 reflection_eye = reflect (incident_eye, N);
	...
	vec3 Is = Ls * Ks * reflected; // final specular intensity

	vec4 regularTexture = texture(cube_texture, reflected);
